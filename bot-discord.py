...
#
#    bot-discrod is distributed in the hope that it will be useful,
...
#
#
@bot.command(brief='Play music from mp3 local file')
async def play(ctx, song_name: str):
	...
	if song_name in files:
		if user.voice:
			...
			raise Exception('User is not in a channel')
	else:
...
async def list(ctx):
	...
	if files:
		...
		await ctx.send("There are not available songs")
#
#
#
